/*
  ...
  cha415
  PA #6
  Implementing the hash table: comes from Femisters Demos
*/
...
HashTable& HashTable::add(string key, string value){
  int h;
  h = Hash(key).hash() % size;
  table[h] = new Link(key, value, table[h]);
  return *this;
}
...
const string HashTable::get(string str) const{
  int h;
  Link *temp;
